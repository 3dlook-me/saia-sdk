...
class Sizechart {
   ...
   * @param {string} [params.customRecommendation] - custom recommendation url part for our clients.
   * For internal usage only or if you have custom url and you know what you're doing
  ...
  getSize(params) {
    ...
    const isCustom = !!params.customRecommendation;

    return this.axios({
      url: `${this.api}${(isCustom) ? 'custom/' : ''}size/recommendation${(isCustom) ? `/${params.customRecommendation}` : ''}/`,
      method: 'get',
      params,
    })
