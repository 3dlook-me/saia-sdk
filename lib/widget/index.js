...
class Widget {
    ...
    this.createPerson()
      ...
      .then((responseSizes) => {
        ...
        if (this.defaults.outputMode === 'params') {
          return this.outputParams(this.defaults.resultsEl, person);
        }

        if (this.defaults.outputMode === 'sizes') {
          return this.outputSizes(this.defaults.resultsEl, person, sizes);
        }
   ...
   */
  static outputSizes(el, person, sizes) {
    if (!el) {
      throw new Error('el is not set');
    }
