...
class API {
   ...
   */
  createPerson(data) {
    const finalFrontImage = data.frontImage || this.data.frontImage;
    const finalSideImage = data.sideImage || this.data.sideImage;
    const finalGender = data.gender || this.data.gender;
    const finalHeight = data.height || this.data.height;
    ...
    return this.getBase64(finalFrontImage)
      .then((frontImage) => {
        frontImageBase64 = frontImage;

        return this.getBase64(finalSideImage);
      })
      .then((sideImage) => {
  ...
  /**
   * Create new instance of a person with metadate
   *
   * @async
   * @example
   * // you can pass user's info here
   ...
   */
  recalculate(id) {
    const finalInstanceUrl = (id) ? `${this.host}${id}/` : this.data.instanceURL;

    if (!finalInstanceUrl) {
      throw new Error('instanceURL is not found');
    }

    return axios({
      url: `${finalInstanceUrl}calculate/`,
      headers: this.headers,
      method: 'get',
    })
      .then((response) => {
